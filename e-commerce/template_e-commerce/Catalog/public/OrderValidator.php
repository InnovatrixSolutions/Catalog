...
class OrderValidator {
    public static function validarTipoPedido(array $data): void {
        ...
        // Si es catalogo → No debe tener datos de dropshipper
        // if ($tipo === 'catalogo' && !empty($data['doc_asesor'])) {
        //     throw new InvalidArgumentException("Los pedidos tipo catálogo no pueden incluir datos de asesor (doc_asesor).");
        // }
    ...
    private static function validarMedioPago(string $medio): void {
        ...
        if (!in_array(strtolower($medio), $mediosValidos)) {
    ...
    private static function validarFormaPago(string $medio): void {
        $mediosValidos = ['nequi','bol','daviplata','mercadoPago','sistecredito','transfiya','otro'];
    ...
    private static function validarMedioPagoAsesor(string $medio): void {
        ...
        $mediosValidos = ['nequi','bancolombia', 'daviplata', 'otro'];
        if (!in_array(strtolower($medio), $mediosValidos)) {
    ...
    public  static function validarDropshipper(array $data, PDO $conexion): void {
        // ========== CAMPOS REQUERIDOS ==========
                ...
                // if(strtolower($data['medio_pago'])=='transferencia'){
                //     self::validarFormaPago($data['forma_pago']);
                // }
                ...
                if (!$asesor) {
                    //insertar asesor automaticamente.
                ...
                //   throw new RuntimeException("Asesor noeee registrado");
                }
                ...
                // if ($asesor['pin'] !== $data['pin_asesor']) {
                //     throw new RuntimeException("PIN incorrecto");
                // }
                // if ($asesor['tipo'] !== 'dropshipper') {
                //     throw new RuntimeException("El asesor no es tipo dropshipper");
                // }
            }
    ...
    public  static function actualizarDatosAsesor(PDO $conexion, string $docAsesor, array $datos): void {
        ...
        // Validaciones específicas
        if (isset($datosFiltrados['pin'])) {
            self::validarPin($datosFiltrados['pin']);
        }
        if (isset($datosFiltrados['telefono_asesor'])) {
            self::validarTelefono($datosFiltrados['telefono_asesor'], 'teléfono');
        }
        if (isset($datosFiltrados['telefono_whatsapp'])) {
            self::validarTelefono($datosFiltrados['telefono_whatsapp'], 'teléfono WhatsApp');
        }
        if (isset($datosFiltrados['email'])) {
            self::validarEmail($datosFiltrados['email']);
        }
        if (isset($datosFiltrados['medio_pago_comision'])) {
            self::validarMedioPagoAsesor($datosFiltrados['medio_pago_comision']);
        }
