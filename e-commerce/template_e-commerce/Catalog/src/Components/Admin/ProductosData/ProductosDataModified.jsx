...
const tipoLista = modeToBackend[mode] || "catalogo";
    ...
    const abrirModal = (item) => {
        ...
        if (item.imagen1) previews.push(item.imagen1);
        if (item.imagen2) previews.push(item.imagen2);
        if (item.imagen3) previews.push(item.imagen3);
        if (item.imagen4) previews.push(item.imagen4);
      
        setImagePreviews(previews);
        setNewImages(files); // Clear files (we don’t have actual File objects for saved URLs)
      };
    ...
    const handleFileChange = (event) => {
      ...
      const handleDrop = (index) => (e) => {
        ...
        const imagesCopy = [...newImages];
      
        // Swap positions
        const [movedPreview] = previewsCopy.splice(fromIndex, 1);
        previewsCopy.splice(index, 0, movedPreview);
      
        const [movedImage] = imagesCopy.splice(fromIndex, 1);
        imagesCopy.splice(index, 0, movedImage);
      
        setImagePreviews(previewsCopy);
    ...
    //       formData.append(`imagen${i + 1}`, img);
    //     });
      
    //     try {
    //       const response = await fetch(`${baseURL}/productoImagePut.php`, {
    //         method: 'POST',
    //         body: formData,
    //       });
    ...
    const handleEditarImagenBanner = async (idProducto) => {
        ...
        // Only include real files in the formData
                    ...
                    <div className="modal-content">
                        ...
                        <div className='sectiontext' style={{ display: selectedSection === 'texto' ? 'flex' : 'none' }}>
                            ...
                            {imagePreviews.map((src, index) => (
                                ...
                                style={{ width: '100px', height: '100px', margin: '5px', border: '1px solid #ccc', cursor: 'grab' }}
                                />
                            ))}
                            </div>
